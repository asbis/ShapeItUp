...
function formatStatusText(status: EngineStatus): string {
  ...
  if (geomInvalid) {
    ...
    return `${headline}\nFile: ${status.fileName || "unknown"}${warnings}\nStats: ${status.stats}${parts}${bbox}${material}${properties}${belowZeroWarn}${currentParams}${timings}\nTime: ${status.timestamp}${SHAPEITUP_VERSION_TAG}`;
  }
  return `${headline}\nFile: ${status.fileName || "unknown"}\nStats: ${status.stats}${parts}${bbox}${material}${properties}${belowZeroWarn}${currentParams}${timings}${warnings}\nTime: ${status.timestamp}${SHAPEITUP_VERSION_TAG}`;
}
