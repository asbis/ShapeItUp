/**
 * Installed-app detector for the MCP server. Ported from
 * packages/extension/src/app-detector.ts so `list_installed_apps` works
 * standalone — without a running VSCode extension. The extension keeps its
 * own copy for its own UI warmup + open-in-app flow; keeping the two files
 * independent means a future detection tweak can ship in one package without
 * forcing a simultaneous release of the other.
 *
 * No vscode dependency. Pure Node built-ins (fs/path/os/child_process).
 */
import * as fs from "fs";
import * as path from "path";
import * as os from "os";
import { execFileSync } from "child_process";
import type { AppId, DetectedApp, ExportFormatForApp } from "@shapeitup/shared";
...
/**
 * Async variant — the one the MCP `list_installed_apps` handler uses. Dedupes
 * concurrent scans (if two MCP calls hit the server before the first finishes,
 * both get the same promise) and populates the sync cache so later
