...
- **`VSCE_PAT` expires (Azure DevOps PATs last a year by default) and its failure is silent-ish.** It is the LAST step, so npm has already published when it fails and the release reads as half-done rather than broken. v1.24.0 and v1.27.0 both failed this way and the Marketplace listing sat at 1.23.0 (8 Jul 2026) for four versions. Fix: mint a new PAT at https://aka.ms/vscodepat (Marketplace → Manage scope), update the `VSCE_PAT` repo secret, then re-run the failed release — the npm step skips an already-published version, so the re-run reaches the Marketplace step. Check `npx @vscode/vsce show shapeitup.shapeitup-vscode` after any release; npm and the Marketplace can drift apart without anything saying so.
