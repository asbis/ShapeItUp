...
- `filletEdge` / `chamferEdge` `(shape, edgeFinder, size)` — round or bevel ONE edge, named by a point on it: `(e) => e.containsPoint([0, -depth / 2, thickness])`. Write the coordinates as expressions — a frozen point stops lying on the edge once the part resizes, and the operation vanishes. Note that OCCT carries a fillet across edges that meet smoothly, so on a rounded outline one pick rounds the whole loop.
- `extrudeFace` / `filletFace` / `chamferFace` `(shape, faceFinder, size)` — push or pull one planar face along its normal (negative pushes in, holes preserved), or round / bevel the edges around it. These are what the viewer writes when you select a face and press Extrude, Fillet or Chamfer. All three take a **FaceFinder**, not an EdgeFinder: `filletFace` reads the face's own wires, which is exact, whereas `e.inPlane(...)` also catches arcs that merely start in the plane and curve away — and OCCT then rejects the operation. Bind the offset to a parameter — `f.inPlane("XY", thickness)`, not `f.inPlane("XY", 8)` — or the operation silently stops matching the moment that dimension changes.
- `joinBodies` / `cutBodies` / `intersectBodies` `(target, tool | tools[])` — Fusion 360's Modify → Combine over whole bodies: union, subtract, keep-only-the-overlap. Prefer these over a bare `.fuse` / `.cut` / `.intersect` when the bodies are MEANT to interact — they measure the result and warn on the three silent failures (a join of bodies that never touch, which returns one "body" of separate lumps; a cut that ate the whole target; an intersect with no overlap, which returns the target unchanged rather than an empty solid). "Keep Tools" needs no argument: a tool that should survive keeps its entry in the returned parts list. These are what the viewer writes when you select two bodies and press Join, Cut or Intersect.
- **Hollowing** — `shellFace(shape, finder, thickness)` leaves the faces the finder MATCHES open and offsets the rest inward; you point at the opening, not at what to keep. More than one match is legitimate (a tube open at both ends). The thickness limit is not a fraction of the part's size — it is the thinnest region the offset passes through, which is often a corner radius: one 60x45x24 box with the top removed caps at 5.0mm with r5 corners and 23mm with sharp ones, same bounding box. Probe rather than predict. `.either([...])` belongs on the bare finder; chaining it after a constraint ANDs instead.
- **Copies** — `joinBodies(arm, arm.clone().mirror("YZ"))` makes one symmetric part; a second entry with the same expression makes a left hand and a right hand. Mirror carries no coordinates at all, so it is the one operation with nothing in it that can go stale. The `.clone()` is required: `.mirror` consumes its receiver and the original is still needed. For repeats use `patterns.repeat(body, patterns.grid(3, 2, 30, 25))` or `patterns.polar(8, 40)` — **not** `patterns.spread(() => body, …)`, whose factory form frees the body on the first placement and throws "This object has been deleted" on the second.
- **Positioning a body** — plain replicad `.translate(x, y, z)` and `.rotate(deg, pivot, axis)`, which is what the viewer writes when you drag the Move triad or a Rotate arc. Rotation needs a pivot, and a pivot must never be frozen coordinates: `rotate(45, [20, 5, 3], …)` is correct the day it is written and silently wrong once a parameter moves the body. Write `shape.boundingBox.center` — it recomputes, so the body keeps turning about its own centre — or omit the pivot entirely for `rotate(deg)`, which turns about +Z through the origin. A self-pivot names the shape twice, so give it a `const` rather than rotating an inline expression. `.rotate(…).translate(…)` turns first and then slides; the other order makes the translation part of the radius. **Transforms consume their receiver**: `.translate` / `.rotate` / `.mirror` / `.scale` all end with `this.delete()` (unlike `.fuse` / `.cut` / `.intersect`, which leave both operands alive), so a shape you transform and also name elsewhere must be `.clone()`d first — otherwise the other use reads a freed object and the render dies with "This object has been deleted" on a line that looks fine. `{ shape: boss }, { shape: boss.clone().translate(-40, 0, 0) }` is what the viewer writes for a copy.
